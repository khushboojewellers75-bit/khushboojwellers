import { necklaceData } from './product.js';
...
function getAllNecklaces() {
  return Object.values(necklaceData).flat();
}
...
function getCategoryFromId(id) {
  if (id.startsWith('chain')) return 'chains';
  if (id.startsWith('pend')) return 'pendants';
  if (id.startsWith('bridal')) return 'bridal';
  if (id.startsWith('dia')) return 'diamond';
  return 'all';
}

function displayNecklaces(category = 'all') {
  const grid = document.getElementById('necklacesGrid');
  let items = category === 'all' ? getAllNecklaces() : necklaceData[category] || [];
